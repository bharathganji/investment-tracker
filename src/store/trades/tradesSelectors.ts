...
export const selectBuyTrades = createSelector(
  [selectTrades],
  (trades) => trades.filter(trade => trade.side === 'buy')
);

export const selectSellTrades = createSelector(
  [selectTrades],
...
export const selectTradesByDateRange = createSelector(
 [selectTrades, (_, startDate: Date, endDate: Date) => ({ startDate, endDate })],
  (trades, { startDate, endDate }) => trades.filter(trade => {
...
export const selectTotalFees = createSelector(
  [selectTrades],
  (trades) => trades.reduce((sum, trade) => sum + trade.fees, 0)
);

export const selectTotalBuyAmount = createSelector(
  [selectBuyTrades],
  (buyTrades) => buyTrades.reduce((sum, trade) => sum + (trade.quantity * trade.price + trade.fees), 0)
);

export const selectTotalSellAmount = createSelector(
  [selectSellTrades],
  (sellTrades) => sellTrades.reduce((sum, trade) => sum + (trade.quantity * trade.price - trade.fees), 0)
);
...
export const selectUniqueAssets = createSelector(
  [selectTrades],
  (trades) => [...new Set(trades.map(trade => trade.asset))]
);
