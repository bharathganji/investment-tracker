...
import { toSerializableTrade } from "@/lib/utils";
import {
  setLoading,
  setError,
  addTrade,
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      if (Array.isArray(trades)) {
        const serializableTrades = trades
          .map((trade) => {
            if (
              typeof trade === "object" &&
              trade !== null &&
              ...
              return {
                ...tradeObj,
                date:
                  typeof tradeObj.date === "string"
                    ? tradeObj.date
                    : new Date(tradeObj.date as string).toISOString(),
  ...
  async (tradeData: Omit<Trade, "id">, { dispatch, rejectWithValue }) => {
    try {
      ...
      const tradeWithRoundedFees = {
        ...validatedTrade,
        fees:
          validatedTrade.fees !== undefined
            ? Math.round(validatedTrade.fees * 100) / 100
            : 0,
      };
      ...
      const tradesData =
        localStorage.getItem("investment-tracker-trades") ?? "[]";
...
export const updateTrade = createAsyncThunk(
  "trades/updateTrade",
  async (
    { id, updatedTrade }: { id: string; updatedTrade: Partial<Trade> },
    ...
    try {
      ...
      const tradesData =
        localStorage.getItem("investment-tracker-trades") ?? "[]";
      const trades: Array<Record<string, unknown>> = JSON.parse(
        tradesData,
      ...
      if (index !== -1) {
        ...
        const updatedTradeObj: Trade = {
          ...
          date: updatedTrade.date
            ? new Date(updatedTrade.date)
            : new Date((trades[index] as { date: string | Date }).date),
          asset:
            updatedTrade.asset ?? (trades[index] as { asset: string }).asset,
          side:
            updatedTrade.side ??
            (trades[index] as { side: "buy" | "sell" }).side,
          quantity:
            updatedTrade.quantity ??
            (trades[index] as { quantity: number }).quantity,
          price:
            updatedTrade.price ?? (trades[index] as { price: number }).price,
          fees:
            updatedTrade.fees !== undefined
              ? Math.round(updatedTrade.fees * 100) / 100
              : (trades[index] as { fees: number }).fees,
          notes:
            updatedTrade.notes ?? (trades[index] as { notes?: string }).notes,
        };
        const serializableTrade = toSerializableTrade(updatedTradeObj);

        localStorage.setItem(
          "investment-tracker-trades",
          JSON.stringify(trades),
        );

        // Update the store
        console.log("Dispatching updateTradeLocal action with:", {
          id,
          updatedTrade: serializableTrade,
        });
