...
export const calculatePortfolioHoldings = createAsyncThunk(
  ...
  async (trades: Trade[], { rejectWithValue }) => {
    try {
...
export const portfolioSlice = createSlice({
  ...
  reducers: {
    setHoldings: (
      state,
      action: PayloadAction<CalculatedPortfolioHolding[]>,
    ) => {
    ...
    updateHolding: (
      ...
      const index = state.holdings.findIndex(
        (holding) => holding.asset === action.payload.asset,
      );
      if (index !== -1) {
    ...
    removeHolding: (state, action: PayloadAction<string>) => {
      state.holdings = state.holdings.filter(
        (holding) => holding.asset !== action.payload,
      );
    },
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        calculatePortfolioHoldings.fulfilled,
        (state, action: PayloadAction<CalculatedPortfolioHolding[]>) => {
          state.loading = false;
          state.holdings = action.payload;
        },
      )
      .addCase(calculatePortfolioHoldings.rejected, (state, action) => {
        ...
        state.error =
          ...
          "Failed to calculate portfolio holdings";
      });
  },
});

export const { setHoldings, addHolding, updateHolding, removeHolding } =
