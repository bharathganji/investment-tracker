import {
  type InvestmentGoal,
  type GoalAnalyticsMetrics,
  type GoalProgressAnalysis,
} from "@/types";
...
export const calculateGoalAnalyticsMetrics = (
  ...
  const activeGoals = goals.filter(
    (goal) => new Date(goal.deadline) > now,
  ).length;
  ...
  const goalsNeedingAttention = goals.filter((goal) => {
    ...
    const timeProgress =
      totalTimeFromStart > 0
        ? ((now.getTime() - startOfYear.getTime()) / totalTimeFromStart) * 100
        : 0;
...
export const calculateGoalProgressAnalysis = (
  ...
  const daysRemaining = Math.max(
    0,
    Math.ceil((deadline.getTime() - now.getTime()) / (1000 * 60 * 24)),
  );
  ...
  const daysSinceStart = Math.ceil(
    (now.getTime() - startOfYear.getTime()) / (1000 * 60 * 24),
  );
  const progressPerDay =
    daysSinceStart > 0 ? goal.progress / daysSinceStart : 0;
  const projectedCompletion =
    progressPerDay > 0
      ? Math.min(100, goal.progress + daysRemaining * progressPerDay)
      : goal.progress;
  ...
  if (
    ...
    recommendedMonthlyContribution =
      monthsRemaining > 0 ? shortfall / monthsRemaining : shortfall;
  }
...
export const calculateGoalStrategicInsights = (
  ...
  const achievableGoals = analyses
    .filter(
      (analysis) => analysis.isOnTrack || analysis.projectedCompletion >= 90,
    )
    .map((analysis) => analysis.goalId);
  ...
  const atRiskGoals = analyses
    .filter(
      (analysis) => !analysis.isOnTrack && analysis.projectedCompletion < 70,
    )
    .map((analysis) => analysis.goalId);
  ...
  const projectedCompletionRate =
    analyses.length > 0 ? totalProjectedCompletion / analyses.length : 0;
