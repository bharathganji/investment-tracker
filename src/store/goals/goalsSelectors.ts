...
} from "@/lib/calculations/goal-calculations";
...
export const selectGoalsWithProgress = createSelector([selectGoals], (goals) =>
  goals.map((goal) => ({
    ...goal,
...
export const selectActiveGoals = createSelector([selectGoals], (goals) =>
  goals.filter((goal) => new Date(goal.deadline).getTime() > Date.now()),
);
...
export const selectTotalTargetAmount = createSelector([selectGoals], (goals) =>
  goals.reduce((sum, goal) => sum + goal.targetAmount, 0),
);

export const selectTotalCurrentAmount = createSelector([selectGoals], (goals) =>
...
export const selectGoalsNeedingAttention = createSelector(
  ...
  (analyses: GoalProgressAnalysis[]) =>
    analyses.filter((analysis: GoalProgressAnalysis) => !analysis.isOnTrack),
);
...
export const selectGoalProgressAnalysisById = createSelector(
  ...
  (analyses: GoalProgressAnalysis[], id: string) =>
    analyses.find((analysis: GoalProgressAnalysis) => analysis.goalId === id) ??
    null,
);
